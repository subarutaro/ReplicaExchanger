...
#include"vector4.hpp"
	...
	return Vector<T,3>(Vector<T,3>(xx,xy,xz)%rhs,
			   Vector<T,3>(yx,yy,yz)%rhs,
			   Vector<T,3>(zx,zy,zz)%rhs);
      }
      Matrix3 operator * (const Matrix3 & rhs) const {
	return Matrix3(Vector<T,3>(xx,xy,xz)%Vector<T,3>(rhs.xx,rhs.yx,rhs.zx),
		       Vector<T,3>(xx,xy,xz)%Vector<T,3>(rhs.xy,rhs.yy,rhs.zy),
		       Vector<T,3>(xx,xy,xz)%Vector<T,3>(rhs.xz,rhs.yz,rhs.zz),

		       Vector<T,3>(yx,yy,yz)%Vector<T,3>(rhs.xx,rhs.yx,rhs.zx),
		       Vector<T,3>(yx,yy,yz)%Vector<T,3>(rhs.xy,rhs.yy,rhs.zy),
		       Vector<T,3>(yx,yy,yz)%Vector<T,3>(rhs.xz,rhs.yz,rhs.zz),

		       Vector<T,3>(zx,zy,zz)%Vector<T,3>(rhs.xx,rhs.yx,rhs.zx),
		       Vector<T,3>(zx,zy,zz)%Vector<T,3>(rhs.xy,rhs.yy,rhs.zy),
		       Vector<T,3>(zx,zy,zz)%Vector<T,3>(rhs.xz,rhs.yz,rhs.zz));
      }
